...
use crate::replay::{
    ...
    EffectOutcomeKind, INTERPRETER_RUNTIME, ReplayHeader, ReplayLogConfig, ReplayLogLine,
    ReplayReadError, ReplayReader, ReplayWriter, RequiredByKind, WASM_RUNTIME, fingerprint_files,
...
pub struct LiveRuntime {
    authorize: Box<dyn FnMut(&str) -> bool>,
...
impl LiveRuntime {
    ...
    ) -> Result<Self, RuntimeEffectError> {
        let mut log = None;
...
impl RuntimeService for LiveRuntime {
    fn authorize(&mut self, check: CapabilityCheck) -> Result<(), RuntimeEffectError> {
        let allowed = (self.authorize)(&check.capability);
...
impl ReplayRuntime {
    pub fn new(mode: ReplayMode, events: VecDeque<ReplayLogLine>) -> Self {
        Self {
            mode,
            events,
            next_seq: 0,
    ...
    pub fn from_log_path(
        log_path: &Path,
        mode: ReplayMode,
    ) -> Result<(Self, ReplayHeader), ReplayReadError> {
        let reader = ReplayReader::from_path(log_path)?;
        verify_program_fingerprint(&reader.header().program_fingerprint)?;
        let (header, events) = reader.into_parts();
        Ok((Self::new(mode, events), header))
    }
    ...
    fn check_effect_event(
        ...
        let should_verify_input =
            request.effect_kind == EffectKind::Read || self.mode == ReplayMode::Verify;
...
pub fn build_replay_header(
    entry_file: &Path,
    project_mode: bool,
    files: impl IntoIterator<Item = PathBuf>,
    runtime_name: &str,
) -> std::io::Result<ReplayHeader> {
...
pub fn is_supported_runtime_name(runtime: &str) -> bool {
    matches!(runtime, INTERPRETER_RUNTIME | WASM_RUNTIME)
}
