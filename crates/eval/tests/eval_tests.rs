...
#[test]
fn eval_array_new_fill_semantics() {
    let val = run_ok(
        "import collections
         fn main() -> Int {
           let xs = collections.Array.new(3, 7)
           xs.len() + xs[0] + xs[1] + xs[2]
         }",
    );
...
#[test]
fn eval_array_new_zero_len_is_empty() {
    let val = run_ok(
        "import collections
         fn main() -> Int {
           let xs = collections.Array.new(0, 1)
           if (xs.is_empty() && xs.len() == 0) { 1 } else { 0 }
         }",
    );
    assert!(matches!(val, Value::Int(1)));
}
...
#[test]
fn eval_array_new_negative_len_runtime_error() {
    let err = run_err(
        "import collections
         fn main() -> Int {
           collections.Array.new(-1, 0).len()
         }",
    );
    assert!(
        err.contains("array_new: length must be >= 0"),
        "unexpected error: {err}"
    );
}

#[test]
fn eval_array_from_list_and_index() {
    let val = run_ok(
        "import collections
         fn main() -> Int {
           let xs = collections.Array.from_list(List.new().push(3).push(4))
           xs[1]
         }",
    );
    assert!(matches!(val, Value::Int(4)));
...
#[test]
fn eval_array_get_some_and_none() {
    let val = run_ok(
        "import collections
         fn main() -> Int {
           let xs = collections.Array.from_list(List.new().push(10).push(20))
           let a = xs.get(1).unwrap_or(0)
           let b = xs.get(-1).unwrap_or(-7)
           let c = xs.get(9).unwrap_or(-9)
           a + b + c
         }",
    );
    assert!(matches!(val, Value::Int(4)));
...
#[test]
fn eval_array_set_and_update_basic() {
    let val = run_ok(
        "import collections
         fn main() -> Int {
           let xs = collections.Array.new(3, 0)
           let ys = xs.set(1, 9).update(2, fn(n: Int) => n + 5)
           ys[1] + ys[2]
         }",
    );
    assert!(matches!(val, Value::Int(14)));
...
#[test]
fn eval_array_set_out_of_bounds_runtime_error() {
    let err = run_err(
        "import collections
         fn main() -> Int {
           collections.Array.new(2, 0).set(3, 1).len()
         }",
    );
    assert!(err.contains("index out of bounds"), "got: {err}");
}
...
#[test]
fn eval_array_update_negative_index_runtime_error() {
    let err = run_err(
        "import collections
         fn main() -> Int {
           collections.Array.new(2, 0).update(-1, fn(n: Int) => n + 1).len()
         }",
    );
    assert!(err.contains("index out of bounds"), "got: {err}");
}
...
#[test]
fn eval_array_index_out_of_bounds_runtime_error() {
    let err = run_err(
        "import collections
         fn main() -> Int {
           let xs = collections.Array.new(2, 0)
           xs[4]
         }",
    );
    assert!(err.contains("index out of bounds"), "got: {err}");
...
#[test]
fn eval_array_immutable_semantics_alias_unchanged() {
    let val = run_ok(
        "import collections
         fn main() -> Int {
           let a0 = collections.Array.new(2, 1)
           let a1 = a0.set(0, 9)
           a0[0] * 10 + a1[0]
         }",
    );
    assert!(matches!(val, Value::Int(19)));
}

#[test]
fn eval_array_collection_first_traversal_and_short_circuit() {
    let val = run_ok(
        "import collections
         fn main() -> Int {
           let xs = collections.Array.from_list(List.new().push(1).push(2).push(3))
           let c = xs.map(fn(n: Int) => n + 1).filter(fn(n: Int) => n > 2).count()
           let a = xs.any(fn(n: Int) => n == 2)
           let b = xs.all(fn(n: Int) => n < 4)
           let f = xs.find(fn(n: Int) => n % 2 == 0).unwrap_or(-1)
           c + (if (a) { 10 } else { 0 }) + (if (b) { 100 } else { 0 }) + f
         }",
    );
    assert!(matches!(val, Value::Int(114)));
}

#[test]
fn eval_array_zip_with_list_deque_and_range() {
    let val = run_ok(
        "import collections
         fn main() -> Int {
           let xs = collections.Array.from_list(List.new().push(1).push(2).push(3))
           let z1 = xs.zip(List.new().push(10).push(20)).count()
           let z2 = xs.zip(collections.Deque.new().push_back(1).push_back(2)).count()
           let z3 = xs.zip((0..<10)).count()
           z1 * 100 + z2 * 10 + z3
         }",
    );
    assert!(matches!(val, Value::Int(223)));
}

#[test]
fn eval_global_array_constructor_surface_is_removed() {
    let err = run_err("fn main() -> Int { Array.new(2, 0).len() }");
    assert!(
