...
contract
  ...
  ensures (match (result) {
    Result.Ok(a) => a.balance.amount == old(acct.balance.amount) - amt.amount
    Result.Err(_) => true
  })
{
  ...
...
Pure by default. Private by default — use `pub` to export. No null — use `Option<T>`. No exceptions — use `Result<T, E>`. Exhaustive pattern matching enforced by the compiler. Pipeline operator (`|>`) and error propagation (`?`) for clean data flow. Convention-based modules — file path determines module path, `import math` binds the namespace `math`, `from math import gcd` binds members directly, and there is no source-level `module Path` declaration. Constructors and variants belong to their owning ADT type: `Type.Variant` is always valid, while bare variants require explicit member import (`from Result import Ok, Err`). Bindings are immutable by default with `let`; at top level, `let` defines a module-local immutable value, while `var` remains local-only for loop-carried mutable state and `x = expr` reassigns an existing mutable local. In v1, lambdas may not capture mutable locals. Collection families are not ambient globals: bring them into scope with `from collections import List, MutableList, ...` for bare use, or refer to them as `collections.List`, `collections.MutableMap`, and so on after `import collections`. Constructors remain type-owned under `collections.*`: `collections.List.new()`, `collections.Map.new()`, `collections.Set.new()`, `collections.BitSet.new(size)`, `collections.Deque.new()`, `collections.MutableList.new()`, `collections.MutableList.from_list(xs)`, `collections.MutableMap.new()`, `collections.MutableMap.with_capacity(capacity)`, `collections.MutableSet.new()`, `collections.MutableSet.with_capacity(capacity)`, `collections.MutableBitSet.new(size)`, `collections.MutableDeque.new()`, and `collections.MutablePriorityQueue.new_min()/new_max()`. Immutable collections are snapshot/value types: `List` keeps query, traversal, and value-style transform methods (`len/get/head/tail/is_empty/concat/reversed/sorted/sorted_by/binary_search`); `Deque` keeps `prepended/appended/popped_front/popped_back`; `BitSet` keeps `test/with_bit/without_bit/toggled` plus value-style set algebra. Mutable collections are the canonical edit path: `MutableList` provides alias-visible dense-index and stack-style updates (`push`/`insert`/`last`/`pop`/`extend`/`get`/`set`/`delete_at`/`remove_at`/`update`) plus in-place `reverse/sort/sort_by`; `MutableMap` and `MutableSet` provide alias-visible mutable key/value and set storage for `Hash + Eq` keys/elements; `MutablePriorityQueue` provides explicit min/max construction with `push/peek/pop/len/is_empty`; `MutableBitSet` uses `test/set/reset/flip` plus `union_with/intersection_with/difference_with/xor_with`; `MutableDeque` owns `push_front/push_back/pop_front/pop_back`. `with_capacity(capacity)` is a minimum live-element hint only. Primitive keys/elements (`Int`, `String`, `Char`, `Bool`, `Unit`) use an internal ordered open-address fast path in mutable workloads; this is an implementation detail, not a separate surface type. For nominal keys like points, derive or implement `Hash + Eq` explicitly, e.g. `type Point derive(Eq, Hash) = { x: Int, y: Int }`. `MutableMap` also exposes `get_or_insert_with(key, fn() => value)` as the canonical compute-if-absent helper for memoization and cache-style code. Traversal constructors are surface-level expressions: `start..<end` for half-open integer ranges and `seed.unfold(step)` for stateful generation. Canonical traversal style is collection-first (`xs.map(...).filter(...).count()` for pipeline reuse, `xs.flat_map(fn(...) => ys)` for one-level lazy flattening, `xs.count(fn(...) => ...)` for direct predicate counts, `xs.contains(value)` for direct membership checks, `xs.frequencies()` for histogram/tally queries). `flat_map` accepts callback results from `Seq`, `List`, `MutableList`, `Deque`, `MutableDeque`, and nominal user types that implement `IntoTraversal<T>`, for example to adapt a wrapper type with `IntoTraversal.into_seq(self)`. For predicate/search traversal, default to `any`/`all`/`find`/`count(predicate)`; use `contains(value)` for direct element membership, `frequencies()` when you need the whole bucket-count map, and reserve `fold` for true accumulation/reduction. Loop control syntax is statement-only: `while (cond) { ... }`, `for (pattern in source) { ... }`, `break`, `continue`; `for` patterns support destructuring but must be irrefutable. Pure no-owner digest helpers live under `hash.*`, with `import hash` and `hash.md5(text)` as the canonical MD5 entry point; `String.md5()` remains available as convenience sugar over the same lowercase hexadecimal digest. `String` surface is Unicode-scalar based: `len`, indexing, `substring`, and `chars()` operate on Unicode scalars. `Char` exposes `to_string()`, `code()`, `is_decimal_digit()`, `to_decimal_digit()`, and `to_digit(radix)`; digit helpers use ASCII digit/letter semantics (`0-9`, `a-z`, `A-Z`), and `to_digit(radix)` requires `radix` in `2..=36`. `Int` includes `pow` as the canonical integer exponentiation method and native bitwise operators `&`, `|`, `^`, `~`, `<<`, `>>` (`>>` is arithmetic; shifts require counts in `0..63`). `Float` exposes `abs()`, `to_int()`, `is_nan()`, `is_infinite()`, and `is_finite()`. Float arithmetic intentionally follows IEEE behavior: `1.0 / 0.0` can produce infinities or `NaN`, `1.0 % 0.0` yields `NaN`, `NaN != NaN`, and ordered comparisons with `NaN` are `false`. `Float` intentionally remains outside `Eq`/`Ord`/`Hash` because `NaN` breaks total equality and ordering laws.
...
| **v0.2** | Module system (convention-based layout, `pub` visibility, namespace imports + `from ... import ...` member imports) ✓, refactor engine (rename, add missing match cases, add missing capability) ✓, refactor transactions (atomic verify-before-apply) ✓, capability enforcement (type-level E0011 + runtime manifest `--caps`) ✓, LSP server (diagnostics, hover, go-to-def, references, completion, code actions, formatting) ✓ | **Complete** |
| **v0.3** | KyokaraIR data structures ✓, HIR→KIR lowering ✓, public single-file WASM run/build/replay ✓, strings/closures/list-deque-bitset Wasm support ✓, property-based testing (`kyokara test --explore`) ✓, SMT verification (restricted fragment), project/package-mode Wasm, remaining collection + witness parity | In progress |
...
API surface design rules live in [docs/rfcs/0001-api-surface-law.md](docs/rfcs/0001-api-surface-law.md), with traversal-surface specifics in [docs/rfcs/0002-collection-first-traversal-surface.md](docs/rfcs/0002-collection-first-traversal-surface.md) and [docs/rfcs/0003-opaque-traversal-constructor-surface.md](docs/rfcs/0003-opaque-traversal-constructor-surface.md), module/capability placement model in [docs/rfcs/0004-module-taxonomy-and-capability-boundaries.md](docs/rfcs/0004-module-taxonomy-and-capability-boundaries.md), loop-control surface policy in [docs/rfcs/0006-loop-control-surface.md](docs/rfcs/0006-loop-control-surface.md), compiler API typed-AST opt-in contract in [docs/rfcs/0007-optional-typed-ast-api-output.md](docs/rfcs/0007-optional-typed-ast-api-output.md), collection freeze in [docs/rfcs/0009-collection-surface-normalization.md](docs/rfcs/0009-collection-surface-normalization.md) and [docs/rfcs/0010-bitset-surface.md](docs/rfcs/0010-bitset-surface.md), static trait semantics in [docs/rfcs/0011-static-trait-system-and-constraint-semantics.md](docs/rfcs/0011-static-trait-system-and-constraint-semantics.md), mutable priority queue policy in [docs/rfcs/0012-mutable-priorityqueue-surface.md](docs/rfcs/0012-mutable-priorityqueue-surface.md), local mutable binding policy in [docs/rfcs/0013-local-var-bindings.md](docs/rfcs/0013-local-var-bindings.md), top-level immutable binding policy in [docs/rfcs/0014-top-level-let-bindings.md](docs/rfcs/0014-top-level-let-bindings.md), and package/dependency distribution design in [docs/rfcs/0015-package-manifests-and-external-library-distribution.md](docs/rfcs/0015-package-manifests-and-external-library-distribution.md). Historical mutable-collection lineage lives in superseded [docs/rfcs/0005-mutable-collection-naming-and-placement.md](docs/rfcs/0005-mutable-collection-naming-and-placement.md) and [docs/rfcs/0008-mutable-map-mutableset-surface.md](docs/rfcs/0008-mutable-map-mutableset-surface.md).
Core dispatch shadow-safety, type-owned constructors, and the canonical import model are documented in [docs/design-v0.md#10-standard-library-v0-minimum](docs/design-v0.md#10-standard-library-v0-minimum).
