...
Most languages are built for humans who happen to use tools. Kyokara is built for AI agents that happen to have human reviewers. Every design decision — from the type system to the runtime — optimizes for machine generation, machine verification, and machine refactoring, while producing code that humans can audit at a glance.

## Why Kyokara?

AI writes most of the code now. But the languages it writes *in* were never designed for this:
...
- **"It worked once" isn't good enough.** There's no way to deterministically replay what happened, or prove it won't do something different tomorrow.

Kyokara fixes all of this.
...
Every effectful execution produces a replay log. Network calls, time reads, database queries — all captured. Run the same log back and get *identical* behavior, deterministically. Debug production incidents by replaying them locally. Audit what your automation actually did.
...
Preconditions, postconditions, and property tests are first-class language constructs — not comments, not docstrings, not a separate test framework. They're checked at runtime by default and can be statically verified by an SMT solver.
...
| **v0.1** | Tree-walking interpreter, runtime contracts, core stdlib, desugaring | Planned |
| **v0.2** | WASM codegen, capability sandbox, deterministic replay | Planned |
| **v0.3** | Property testing, SMT verification, refactor engine, LSP, formatter | Planned |
